...
≡ _script_
https://github.com/AkariOfficial/AkariBot

wew = fs.readFileSync(`./src/logo.jpeg`)
_───────────────────────_`
			dtod = "6281211651261@s.whatsapp.net" //jangan diganti tar error
			menunya = `\`\`\`────「 ${botName} 」────\`\`\`
					...
					if ((isMedia && !Lan.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								fs.unlinkSync(media)
								costum('[❗] SEDANG DIPROSES WAITT', text, tescuk, cr)
							})
							.on('end', function () {
								console.log('Finish')
								dila.sendMessage(from, fs.readFileSync(ran), sticker, { quoted: Lan})
